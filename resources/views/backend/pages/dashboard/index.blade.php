...
<div class="main-content-inner">
    ...
    <div class="row mt-2">
        ...
        <div class="col-sm-6">
          <div class="card shadow-lg mb-2" style="background: orange;color:white;height:150px">
            <div class="card-body">
                <center>
                    <h5 class="card-title">Total Transaksi Destinasi {{ $item->nama }}</h5>
                    @php
                        $dataTotal = \App\Models\Pesan::where('id_destinasi',$item->id)->get()->count();
                    @endphp
                    <p class="card-text" style="color: white">{{ $dataTotal }}</p>
                </center>
            </div>
          </div>
        </div>
...
function printReport(){
    window.open(window.location.href.replace("admin",'admin/print'), '_blank');
}
